...
class EncoderDecoder(Chain):
    ...
    def __init__(self, vsize_enc, vsize_dec,
       ...
       Summary: Initialise the model with forward (lstm_enc) and backward (lstm_rev_enc) LSTM layers of depth nlayers_enc
       Steps:
       -construct a list of forward and reverse LSTM layer item labels.
       -Iterate through each item label and "add_link", which associates the item label with an LSTM layer object with input and output size of n_units.
       -Chainer inteprets these layers sequentially, each one is a child of the previous, thus implicitly constructing the computational graph for this model
        ...
        The EmbedID layer takes one-hot encodings of the vocabulary, of size vsize_dec, and transforms them to an array of size 2*n_units. The bidirectional LSTM is simply two LSTMs of size n_units which get concatenated to give an array of shape 2*n_units and so input words need to be converted to an object of this shape, hence the output dimension of EmbedID. The LSTM layer of the decoder takes the word sequence inputs from language A as a 2*n_unit object and maps these to activations of word sequence representations in language B, also as a 2*n_unit dimensional vector. The Linear layer transforms this 2*n_units representation back to a one-hot encoding of the word sequence in language B.
    ...
    '''
        ...
        Summary: set_decoder_state updates variables that store the weights for the newly computed c_state (cell states of the LSTM) and h_state (outputs at the previous timestep) from the encoder.
        steps: 1. checks for GPU flag and uses cuda.cupy if true or numpy if false. 2. Retrieve the last layer from the lstm_enc for h_state and c_state. 3. Concatenate the forward and backward lstm layers. 4. Update the decoder object lstm_dec with the hidden and context layers from the encoder.
    ...
    def feed_lstm(self, word, embed_layer, lstm_layer_list, train):
        ...
        # add dropout layer
        embed_id_drop = F.dropout(embed_id, ratio=dropout_ratio)
        # feed into first LSTM layer
        hs = self[lstm_layer_list[0]](embed_id_drop)
        # add dropout layer
        hs_drop = F.dropout(hs, ratio=dropout_ratio)
        # feed into remaining LSTM layers
        for lstm_layer in lstm_layer_list[1:]:
            hs_drop = self[lstm_layer](hs_drop)
    ...
    def encode_list(self, in_word_list, train=True):
        ...
        for f_word, r_word in zip(var_en, var_rev_en):
           ...
           The word is encoded both forwards and backwards, with a left-to-right and a right-to-left LSTM.
            '''
    ...
    def encode_decode_train(self, in_word_list, out_word_list, train=True):
        ...
        for next_word_var in var_dec[1:]:
            ...
            if self.attn == NO_ATTN:
                ...
                # __QUESTION Add attention
                # concatenate current state and prev state
            ...
            Here the cross entropy of the predicted output is calculated as a loss measure. The cross-entropy is a measure of how distinct two probability distributions are in terms of bits. If this is low the two distributions are hard to distinguish, in other words the predicted distribution is approximately equal to the true distribution.
            '''
